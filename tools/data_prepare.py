...
class get_label_details(object):
    ...
    def __init__(self,cfg,args,logger):
        ...
        self.frame_num = 0
        self.tuple_dict = {} # 存放每个视频的tuple信息
        self.label_parsed = [] # 存放逐帧的标签
    ...
    def get_label(self):
        ...
        # convert_dict = {"m30":"M-30","m30_hd":"M-30-HD","urban":"Urban1"}
        ...
        if self.dataset_group.lower() == "m30":
            ...
            self.frame_num = int(contents[-1].split(" ")[1]) # 该视频中有标注的帧的总数
            self.label_parsed = [[] for i in range(self.frame_num)]
...
def dump_pic(video_full_name, video_name, save_root='',raw_root=''):
    save_root = "/mnt/data_ssd1/xyc/video_data"
    raw_root = "/home/xyc/video_data/Collected_live_videos/standard_split"
    video_path = os.path.join(raw_root, video_full_name, "concat", "test", "%s_test.mp4"%(video_name))
    ...
    while True:
        ...
        if not ret:
            break
        count += 1
        cv2.imwrite(os.path.join(save_path, "%d.jpg"%(count)), frame)
