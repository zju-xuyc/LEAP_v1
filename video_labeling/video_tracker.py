...
def make_parser():
    ...
    parser.add_argument("--sampling_rate", default=1, type=int, help="frame rate (fps)") # 这个一般不需要动
    parser.add_argument(
        "--fp16",
        dest="fp16",
        default=False,
    ...
    parser.add_argument("--video_path", type=str, default="/home/xyc/video_data/Collected_live_videos/standard_split/Square_Northeast/concat/val/square_val.mp4", help="video path")
    parser.add_argument("--mask_path", type=str, default="./masks/square_mask.jpg", help="video path")
        ...
        if flag:
            ...
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > args.min_box_area: # and not vertical:
                    online_tlwhs.append(tlwh) # 位置
                    online_ids.append(tid)    # Id
                    online_scores.append(t.score) # 置信度
                    online_velocity.append([xv, yv]) # 速度
                    online_c_ids.append(t.c_id_list[-1]) # 车辆类型
                    results.append(
...
def get_k_frame_label(video_path,mask_image,k=1000):
    # 按照指定的视频以及选取的K帧进行伪标签的生成
    # save_path 是标签的存储位置
    # 加上遮蔽 mask_image 
    args, _ = make_parser().parse_known_args() # 防止出现未知参数的错误
    cfg.freeze()
    set_seed(cfg.SOLVER.SEED)
    # os.environ["CUDA_VISIBLE_DEVICES"] = cfg.MODEL.DEVICE_ID
